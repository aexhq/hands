...
swap_bytes="${{AEX_SWAP_BYTES:-{DEFAULT_SWAP_BYTES}}}"
...
ENV DEBIAN_FRONTEND=noninteractive \
    ...
    AEX_HAND_LISTEN=0.0.0.0:{AGENT_PORT}
...
/// Packs the build context: `Dockerfile`, `hand-boot.sh`, and the aarch64 guest binary.
...
pub struct PublishConfig {
    /// Image name (one per shape variant), e.g. `aex-hands-dev-1gb`.
    pub name: String,
...
pub async fn publish(
    ...
    let arn = if let Some(arn) = exists {
        sdk.update_microvm_image()
            ...
            .description(format!("aex hand guest ({} MiB shape)", cfg.memory_mib))
...
mod tests {
    ...
    fn the_zip_is_deterministic_and_contains_exactly_the_context() {
        ...
        let names: Vec<String> = (0..ar.len())
            ...
            .collect();
        assert_eq!(names, vec!["Dockerfile", "hand-boot.sh", "hand-guest"]);
