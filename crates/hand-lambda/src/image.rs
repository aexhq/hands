...
/// supervisor binary has only kill/setuid/setgid/net-bind-service capability and irreversibly
/// drops Tool children to `agent`; arbitrary code never shares the control UID. `CAP_KILL` is
/// required because Linux does not grant a parent a special right to signal a child after that
/// child changes to UID 1000. `CAP_NET_BIND_SERVICE` lets only the root-owned supervisor binary
/// bind the reserved provider port after boot raises the unprivileged port floor above it.
#[must_use]
...
RUN chown root:hand /usr/local/bin/hand-guest \
    ...
    && setcap cap_kill,cap_setuid,cap_setgid,cap_net_bind_service=ep /usr/local/bin/hand-guest \
    ...
    && test "$(getcap -r / 2>/dev/null)" = "/usr/local/bin/hand-guest cap_kill,cap_setgid,cap_setuid,cap_net_bind_service=ep"
...
mod tests {
    ...
    fn the_boot_script_pins_dns_and_drops_to_the_supervisor() {
        ...
        assert!(df.contains("setcap cap_kill,cap_setuid,cap_setgid,cap_net_bind_service=ep"));
        ...
        assert!(df.contains(
            "getcap -r / 2>/dev/null)\" = \"/usr/local/bin/hand-guest cap_kill,cap_setgid,cap_setuid,cap_net_bind_service=ep"
        ));
