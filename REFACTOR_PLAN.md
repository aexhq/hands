...
## Execution log (2026-08-22, this branch: `codex/code-md-refactor`)

Phases 0–4 implemented end to end; every commit left `cargo fmt --check`, `cargo clippy
--workspace --all-targets -D warnings`, and the Windows-runnable test suite green (~130 tests).
Linux-only guest/image/tool-runner suites verify in CI on push (no WSL on this machine).

Deviations from the written plan, with reasons:
- `rebuild_due` was wired (new `image rebuild-due` CLI subcommand), not deleted — it encodes a
  real base-image expiry rule.
- `expire_lease` keeps its `now_ms` parameter: the DynamoDB impl genuinely uses it for the
  capacity `updated_at_ms`; only the memory double discards it. The audit finding was wrong.
- `control_error` dedupe kept the sanitized (client.rs) Capacity message — provider text must
  not reach the public contract.
- Canary `ObserveRequest`/`SandboxExecutionRequest` still build via `serde_json::from_value`
  pending Brain-side constructors; the brain-aws `SandboxFileRequest` round-trip was replaced
  with a struct literal.
- Guest typed target fields: deferred to Brain-side constructible types (see annotated bullet).
- Extraction detail: module splits import siblings through crate-root `pub(crate) use module::*`
  globs (`use crate::*` / `use super::*` in members). Deliberate for mechanical splits; tighten
  to explicit imports opportunistically when files are next touched.
...
- [x] **Clock fail-open**: `hand-brain-aws/src/lib.rs:3360` `now_ms()` returns 0 on pre-epoch
  clock; every expiry check (`expires_at_ms <= now_ms()`) then passes. Fail closed
  (`expect` or `HandResult`).
- [x] **Signed sandbox id fallback**: `hand-brain-aws/src/lib.rs:3349-3354` `strip_prefix(...)
...
- [x] **Non-canonical spec digest**: `hand-core/src/materialization.rs:231-238` hashes
...
- [x] **Lease serde hazard**: `hand-core/src/materialization.rs:255-260` `recovery_attempt` is
...
- [x] **Panicking task leaves op Running forever**: `hand-guest/src/hand.rs:745-766, 1254-1275`
  ...
  `Err(_) => return` branch.
- [x] **Silent terminal drop**: `hand-guest/src/hand.rs:1583-1585` missing metadata discards a
  completed result with no log. Add `tracing::warn!`.
- [x] **Oversize response closes socket silently**: `hand-guest/src/server.rs:232-260` encode
...
- [x] **Gateway env fail-open on non-UTF-8**: `hand-egress-gateway/src/config.rs:82,177,187`
  `NotUnicode` is treated as unset — an invalid deny list silently disappears. Use `var_os` /
  match `NotUnicode` → `ConfigError::Invalid`.
- [x] **tool-runner validates ceiling after execution + can emit no frame**:
...
- [x] **Release gate aborts on transient blips**: `hand-lambda/src/canary.rs:1183-1216`
...
- [x] **Guest reply mismatch retried forever**: 16 sites in `hand-brain-aws` (`lib.rs:1441-2287`)
...
- [x] **HTTP 409 for every error**: `hand-guest/src/server.rs:560-573` — add
  `status_for(HandErrorCode)` in `errors.rs` (400/404/409/413/503).

## Phase 1 — Deletions (zero-risk, shrink before splitting)

- [x] **Delete `hand-core/src/target.rs` entirely** (571 lines): a complete second target state
...
- [x] Dead launch paths: `hand-lambda/src/control.rs:348-364` `Control::run` and
...
- [x] `hand-lambda/src/image.rs:629-659` `rebuild_due`: documented as *the* base-image expiry
  safeguard, wired to nothing. Wire it into publish/CI or delete it (and fix `image.rs:9-12` docs).
- [x] Unused deps: gateway `hex`, `sha2`, `tempfile`; hand-wire `serde_json`; hand-core dup
  dev-dep `serde_json`; hand-brain-aws `tracing-subscriber` (keep `tracing` — Phase 4 uses it).
- [x] Guest dead code: `config.rs` `binding_dir` field (+`create_dir_all`, +hardcoded test copy);
...
- [x] hand-lambda dead layer: `ProvideErrorMetadataExt` (`image.rs:709-711`), `detail()`
  (`image.rs:696-707`, lossy re-impl of `control.rs::sdk_error`).
- [x] Stale docs: `hand-lambda/src/lib.rs:14-19`, `launch.rs:95`, `Cargo.toml:4` describe
...
**In-repo, Rust:**
- [x] **New `hand-policy` crate (D5)** — leaf vocabulary crate, all four items in one move:
...
- [x] **`hand_core::page`**: promote `Page<T>`/`PageIdentity` from target.rs before deleting it;
  use in `TargetPage`, `LiveFilePage`; one `MAX_PAGE` const (today `100` appears as literal 4×).
- [x] `control_error` exists twice and diverged (`hand-brain-aws/lib.rs:3461` vs
...
- [x] DynamoDB helpers `s`/`n`/`conditional_failure`/`storage_error` duplicated between
  `registry.rs:974-995` and `definitions.rs:339-368` → small `dynamo` module generic over error.
- [x] One `shard_index` helper for the two divergent 64-shard derivations
  (`hand-brain-aws/lib.rs:685-691` vs `2341-2345`).
- [x] One AWS `SdkConfig` per process: `hand-lambda` bin loads two configs
...
- [x] Port 8080 / uid 1000/1001 / `/usr/local/lib/hand/*` / fd 3 / 4096 headroom are hardcoded
...
- [x] `image/hand-boot.sh` duplicates `image.rs::boot_script()` with zero coverage — loop the
  existing assertion list over both (`include_str!`), same pattern as the two-Dockerfile test.
- [x] Tool IPC contract specified 3× (`tool-runner.mjs`, `process.rs:218-233,559-580`,
...
- [x] Pinned Brain revision written 6× — make `scripts/pinned-brain-revision.sh` emit
...
- [x] **`hand-brain-aws/src/lib.rs` (4,382 → ~9 modules)**. Zero-coupling extractions first:
...
- [x] **`hand-guest/src/hand.rs` (2,748 → façade + sub-modules)**. `Hand` holds 7 independent
...
- [x] **`hand-core/src/materialization.rs` (1,829 → `materialization/` dir)**:
...
- [x] **`hand-lambda/src/canary.rs` (1,471)**: three copy-pasted canary programs. Extract a
...
- [x] **`hand-egress-gateway/src/proxy.rs::handle_http`** (134-226): give `ProxyError` one
...
- [x] **`image.rs::publish`**: split into `upload_context` / `resume_existing_publication` /
  ...
  resume).
- [x] **`process.rs` dedupe**: one `run_execution` wrapper (execute_bundle/execute_shell are
  identical), one `Teardown::abort_all` (4 copies), one generic `settle` (3 copies).
- [x] **`scripts/test-guest-deadline.mjs`**: one `sealed(value)` digest helper (5 copies), one
...
**In-repo:**
- [x] Guest `Config`: replace `Option<tool_boundary_library>` + `Option<tool_identity>` with
...
- [>] Guest `ArmedTarget`/`TargetSnapshot` typed fields: NOT done in-repo. Brain's generated
  protocol uses per-struct newtypes (e.g. `OperationEnvelopeGeneration`), so storing one typed
  value cannot delete the per-projection parses; folded into the Brain-backlog "constructible
  protocol types" item instead.
- [x] hand-core: private fields + `#[serde(try_from)]` on `TargetKey`/`TargetSpec`/
...
- [x] Registry trait split: `TargetReservations { acquire, install, expire_lease }` (all
...
- [x] hand-brain-aws error handling: 63 `map_err(|_| ...)` sites drop the cause and the crate
  ...
  `unreachable!`s).
- [x] `image.rs` stops bypassing `Control`'s error taxonomy via `control.sdk()` — image-plane
...
- [x] Canary type safety: struct literals instead of `serde_json::from_value(json!({...}))`
...
- [x] hand-brain-aws smaller typed fixes: `prepared_targets` string-prefix keys →
...
- [x] hand-wire policy relocation: covered by the `hand-policy` crate in Phase 2 (D5).
- [x] **Gone/Terminated → `Closed { disposition }` (D7 — promoted from deferred)**: merge in
...
- [x] Gateway small: re-export `MAX_DESTINATIONS`/`MAX_GENERATION_LIFETIME_MS` (minters can't see
...
- [x] `#![forbid(unsafe_code)]` in hand-core (contains none).
